...
class AmplifyAuthCognitoMethodChannel extends AmplifyAuthCognito {
  ...
  SignUpResult _formatSignUpResponse(Map<String, dynamic> res, method) {
    if (res.containsKey("nextStep")) {
      ...
      throw (AmplifyDartExceptions.formatException(
          methodName: method, field: "nextStep"));
    }
  }
  ...
  ResendSignUpCodeResult _formatResendSignUpResponse(
    ...
    if (res.containsKey("codeDeliveryDetails")) {
      ...
      throw (AmplifyDartExceptions.formatException(
          methodName: method, field: "codeDeliveryDetails"));
    }
  }
  ...
  ResetPasswordResult _formatResetPasswordResponse(Map<String, dynamic> res) {
    if (res.containsKey("nextStep")) {
      ...
      throw (AmplifyDartExceptions.formatException(
          methodName: "resetPassword", field: "nextStep"));
    }
  }

  AuthSession _formatSessionResponse(Map<String, dynamic> res) {
    AWSCredentials credentials = res.containsKey("credentials")
        ? AWSCredentials.init(creds: res["credentials"])
        : null;
    AWSCognitoUserPoolTokens tokens = res.containsKey("tokens")
        ? AWSCognitoUserPoolTokens.init(tokens: res["tokens"])
        : null;
  ...
  void _throwError(PlatformException e) {
    ...
    e.details.forEach((k, v) => {
          if (cognitoExceptions.contains(k))
            ...
            {
              eMap.putIfAbsent(
                  "UNRECOGNIZED EXCEPTION", () => "See logs for details.")
            }
        });
